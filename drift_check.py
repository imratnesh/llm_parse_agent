...
def check_mean_drift(data, expected_mean, threshold=1.0):
    ...
    print(f"Actual mean: {actual_mean}, Expected mean: {expected_mean}, Drift: {drift}")
    return drift <= threshold

if __name__ == "__main__":
    # Example: python drift_check.py --csv sample_data.csv --column age_int --expected_mean 30 --threshold 0.0
    parser = argparse.ArgumentParser()
    parser.add_argument('--csv', type=str, required=True, help='CSV file path')
    parser.add_argument('--column', type=str, required=True, help='Column name to check')
